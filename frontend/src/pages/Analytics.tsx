...
const Analytics: React.FC = () => {
  ...
  const [goalProjection, setGoalProjection] = useState<any>(null);
  ...
  const combinedData = activeMetric === 'weight' && showForecast && forecastData.length > 0
    ? [
        ...data.map(d => ({ timestamp: d.timestamp, value: d.value })),
        ...forecastData.map(f => ({ timestamp: f.date, forecasted_weight: f.predicted_weight }))
      ]
    : data.map(d => ({ timestamp: d.timestamp, value: d.value }));
    ...
    <div className="space-y-8 animate-in fade-in duration-700">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <div className="lg:col-span-3 glass-panel p-8 rounded-[3rem] border border-white/5 h-[500px] relative overflow-hidden group">
          ...
          <div className="mb-10 flex items-center justify-between">
            ...
            <div className="flex items-center space-x-4">
              {activeMetric === 'weight' && (
                ...
                >
                  {showForecast ? 'Hide Neural Forecast' : '🔮 Show Neural Forecast'}
                </button>
              )}
              {isLoading && (
          ...
          <ResponsiveContainer width="100%" height="75%">
            <AreaChart data={combinedData}>
              ...
              />
              {activeMetric === 'weight' && showForecast && (
                <Line 
                  type="monotone" 
                  dataKey="forecasted_weight" 
                  stroke="#22d3ee" 
                  strokeDasharray="5 5" 
                  strokeWidth={3}
                  dot={{ r: 4, fill: '#22d3ee' }}
        ...
        <div className="space-y-6">
          ...
          {activeMetric === 'weight' && showForecast && goalProjection && (
            <div className="p-6 rounded-[2.5rem] bg-gradient-to-br from-cyan-500/10 to-indigo-500/10 border border-cyan-500/20 relative overflow-hidden animate-in slide-in-from-bottom-2 duration-500">
              <div className="flex items-center space-x-2 mb-3">
                <Target className="text-cyan-400 animate-pulse" size={16} />
                <span className="text-[10px] font-black uppercase text-cyan-400 tracking-wider">Goal projection</span>
              </div>
              <p className="text-xs text-white font-bold italic leading-relaxed">
                {goalProjection.achievable 
                  ? `On track to hit goal weight of ${goalProjection.goal_weight}kg by ${goalProjection.projected_date || 'N/A'}!`
                  : `Goal not achieved in 30-day window. Adapt nutrition/volume to increase rate.`
                }
              </p>
              <div className="mt-4 flex justify-between text-[10px] text-slate-400 font-bold uppercase tracking-wider">
                <span>Confidence: {(goalProjection.confidence * 100).toFixed(0)}%</span>
                <span>{goalProjection.days_remaining} Days Left</span>
              </div>
            </div>
          )}
