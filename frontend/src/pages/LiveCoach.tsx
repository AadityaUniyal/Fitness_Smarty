...
const LiveCoach: React.FC = () => {
  ...
  const startSession = async () => {
    ...
    setLoading(true);
    setIsActive(true);
    setTranscription('Secure live coach relay is not configured yet. Gemini Live cannot run in the browser without exposing the API key.');
    setLoading(false);
    setIsActive(false);
    return;
    ...
    const sessionPromise = ai.live.connect({
        ...
        onmessage: async (message: LiveServerMessage) => {
          ...
          if (message.toolCall) {
            for (const fc of message.toolCall.functionCalls) {
              ...
              if (fc.name === 'requestSessionStop') {
                setTranscription("Uplink termination requested. Shutting down...");
                setTimeout(() => stopSession(), 1500);
                sessionPromise.then(s => s.sendToolResponse({
                  functionResponses: [{ id: fc.id, name: fc.name, response: { result: "SESSION TERMINATING" } }]
                }));
              }
            }
          }
      ...
      config: {
        ...
        HANDS-FREE CONTROL: If the user says "Smarty, Stop", "End session", or similar termination requests, use the 'requestSessionStop' tool immediately.`,
        tools: [{
          functionDeclarations: [
            ...
            },
            {
              ...
              description: 'Terminates the live session based on user voice command.',
