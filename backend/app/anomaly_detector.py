...
Uses simple, robust rolling statistics (Z-score and IQR) to identify
potential fat-fingered logs.
"""
...
def check_single_log_anomaly(
    ...
    """
    Checks if a newly logged value is highly anomalous compared to historical
    entries.
    ...
    if not history:
        ...
        if metric_type == "calories" and (new_value < 50 or new_value > 8000):
            return {
                "is_anomaly": True,
                "reason": (
                    f"Value {new_value} kcal is outside typical physiological "
                    "limits (50-8000 kcal)."
                )
            }
        if metric_type == "weight" and (new_value < 30 or new_value > 250):
            return {
                "is_anomaly": True,
                "reason": (
                    f"Value {new_value} kg is outside plausible human "
                    "limits (30-250 kg)."
                )
            }
        return {
            "is_anomaly": False,
            "reason": "No history available to flag anomaly."
        }

    # We use IQR for calories (often right-skewed) and Z-score/IQR for weight
    ...
    if metric_type == "weight":
        ...
        if pct_change > 0.05:  # Change of > 5% in one log
            return {
                "is_anomaly": True,
                "reason": (
                    f"Weight change of {pct_change*100:.1f}% is physically "
                    "implausible for a single log interval."
                )
            }
        ...
        if std > 0.1:  # Only if there's enough variation
            ...
            if z_score > z_threshold:
                is_anomaly = True
                reason = (
                    f"Weight {new_value}kg is a Z-score outlier "
                    f"(Z={z_score:.2f}, mean={mean:.1f}kg, std={std:.2f}kg)."
                )
        ...
        if iqr > 100:  # Avoid flagging standard variances
            ...
            if new_value > upper_limit:
                is_anomaly = True
                reason = (
                    f"Calorie log of {new_value} kcal exceeds the statistical "
                    f"upper threshold of {upper_limit:.0f} kcal (IQR range: "
                    f"{q25:.0f}-{q75:.0f} kcal)."
                )
            elif new_value < lower_limit:
                is_anomaly = True
                reason = (
                    f"Calorie log of {new_value} kcal is below the statistical "
                    f"lower threshold of {lower_limit:.0f} kcal."
                )
        else:
            ...
            if new_value > mean * 2.5:
                is_anomaly = True
                reason = (
                    f"Calorie log of {new_value} kcal is over 2.5x your typical "
                    f"mean intake ({mean:.0f} kcal)."
                )

    return {
        "is_anomaly": is_anomaly,
        "reason": (
            reason if is_anomaly else "Log is within standard statistical bounds."
        )
    }
