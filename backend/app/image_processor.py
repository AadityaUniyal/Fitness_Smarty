...
class ImageProcessor:
    """
    Handles image validation, preprocessing, and optimization for computer
    vision analysis
    """
    ...
    def validate_image(self, image_bytes: bytes) -> Dict[str, Any]:
        ...
        if file_size < self.MIN_FILE_SIZE:
            raise ImageValidationError(
                f"Image too small: {file_size} bytes "
    ...
    def optimize_for_analysis(self, image_bytes: bytes) -> bytes:
        ...
        if aspect_ratio > 1:
            ...
            if new_height < self.MIN_HEIGHT:
                ...
                # Cap width at target if it exceeds due to minimum height
                # enforcement
            ...
            if new_width < self.MIN_WIDTH:
                ...
                # Cap height at target if it exceeds due to minimum width
                # enforcement
    ...
    def store_image(self, image_bytes: bytes, user_id: str) -> str:
        """
        Store image. In production, uploads to configured Object Storage
        (S3/Cloudflare R2/GCS) and returns a signed URL. Falls back to Base64
        data URL in development.
    ...
    def process_meal_image(
        ...
        original_dims = (
            validation_result['width'], validation_result['height']
        )
        return {
            'success': True,
    ...
    def assess_image_quality(self, image_bytes: bytes) -> Dict[str, Any]:
        ...
        if image.mode == 'RGB':
            ...
            if avg_brightness < 50:
                quality_issues.append(
                    "Image too dark - consider better lighting"
                )
                quality_score -= 20
            elif avg_brightness > 200:
                quality_issues.append(
                    "Image too bright - may be overexposed"
                )
                quality_score -= 15
        ...
        if aspect_ratio > 3 or aspect_ratio < 0.33:
            quality_issues.append(
                "Unusual aspect ratio - ensure full meal is visible"
            )
            quality_score -= 10
    ...
    def _generate_quality_recommendations(self, issues: list) -> list:
        ...
        if any('resolution' in issue.lower() for issue in issues):
            recommendations.append(
                "Use a higher resolution camera or move closer to the meal"
            )
        ...
        if any('aspect ratio' in issue.lower() for issue in issues):
            recommendations.append(
                "Capture the entire meal in frame without extreme cropping"
            )
